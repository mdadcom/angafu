...
def valides(request, confirme_id):
    ...
    if request.method == 'POST':
        ...
        auth_token = '274f11ff35157263401a21caaf59b701'
        twilio_phone_number = '+14242654790'
        
        client = Client(account_sid, auth_token)
        
        message = f'Votre ticket de la compagnie {", ".join(str(res.societe.first().nom) for res in confirme.reservation.all())} est confirmé avec succès pour le trajet {", ".join(str(res.societe.first().destination.nom) for res in confirme.reservation.all())}-{", ".join(str(res.destination.nom) for res in confirme.reservation.all())} à {", ".join(str(res.time.time) for res in confirme.reservation.all())}. Numéro du ticket : {numticket}, chaise : {numchaise}.'
